...
import { ensureTrailingSlash, trimTrailingSlashes } from '../../../src/util/Util';
...
class SimpleDataAccessor implements DataAccessor {
  ...
  public async getNormalizedMetadata(identifier: ResourceIdentifier): Promise<RepresentationMetadata> {
    if (this.data[ensureTrailingSlash(identifier.path)]) {
      return this.getMetadata({ path: ensureTrailingSlash(identifier.path) });
    }
    return this.getMetadata({ path: trimTrailingSlashes(identifier.path) });
  }
...
describe('A DataAccessorBasedStore', (): void => {
  ...
  describe('adding a Resource', (): void => {
    ...
    it('will error if it gets a non-404 error when reading the container.', async(): Promise<void> => {
      const resourceID = { path: `${root}container` };
      accessor.getNormalizedMetadata = async(): Promise<any> => {
    ...
    it('does not allow adding resources to existing non-containers.', async(): Promise<void> => {
      ...
      await expect(store.addResource(resourceID, representation))
        .rejects.toThrow(new MethodNotAllowedHttpError('The given path is not a valid container.'));
    });
  ...
  describe('modifying a Representation', (): void => {
    it('will 404 if the identifier does not contain the root.', async(): Promise<void> => {
      await expect(store.modifyResource({ path: 'verybadpath' }, representation))
        .rejects.toThrow(NotFoundHttpError);
    });

    it('will pass the input to the DataAccessor.', async(): Promise<void> => {
      await expect(store.modifyResource({ path: `${root}resource` }, representation))
        .rejects.toThrow(new Error('modify'));
    });
  });
