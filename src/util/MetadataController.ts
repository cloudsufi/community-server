...
export class MetadataController {
  ...
  public generateResourceQuads(subject: NamedNode, isContainer: boolean): Quad[] {
    const quads: Quad[] = [];
    if (isContainer) {
      quads.push(DataFactory.quad(subject, toNamedNode(RDF.type), toNamedNode(LDP.Container)));
      quads.push(DataFactory.quad(subject, toNamedNode(RDF.type), toNamedNode(LDP.BasicContainer)));
    }
    quads.push(DataFactory.quad(subject, toNamedNode(RDF.type), toNamedNode(LDP.Resource)));
