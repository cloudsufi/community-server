...
export class ExtensionBasedMapper implements FileIdentifierMapper {
  ...
  public async mapUrlToFilePath(identifier: ResourceIdentifier, contentType?: string): Promise<ResourceLink> {
    ...
    if (/\$\.\w+$/u.test(path)) {
      throw new UnsupportedHttpError('Identifiers should not match /\\$\\.\\w+$/.');
    }
    ...
    if (!contentType) {
      ...
      try {
        ...
        actualName = files.find(
          (file): boolean => new RegExp(`^${name}(?:\\$\\..+)?$`, 'u').test(file),
        );
      } catch {
      ...
      return {
        identifier,
        filePath: joinPath(folder, actualName),
        contentType: this.getContentTypeFromExtension(actualName),
      };
    }
